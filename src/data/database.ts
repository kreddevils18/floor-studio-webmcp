...
interface FloorStudioDb extends DBSchema {
  ...
  timeline: { key: string; value: AgentTimelineEvent; indexes: { byProject: string } }
...
export async function clearDatabaseForTests() {
  ...
  const stores = ['settings', 'projects', 'revisions', 'drafts', 'timeline', 'tickets', 'previews', 'blobs'] as const
...
export async function saveInitialProject(project: ProjectDocumentV1) {
  ...
  await tx.objectStore('revisions').put({
...
export async function saveBlob(ref: string, blob: Blob) {
  await (await database()).put('blobs', blob, ref)
}
export async function getBlob(ref: string) {
  return (await database()).get('blobs', ref)
}
